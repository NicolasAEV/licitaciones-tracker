...
export const metadata = {
  title: "Licitaciones en Seguimiento | Licitaciones Tracker",
};

export default async function TrackedTendersPage() {
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-8">
        <h1 className="mb-1 text-2xl font-bold text-zinc-900">
          Licitaciones en Seguimiento
        </h1>
        <p className="text-sm text-zinc-500">
          Lista de licitaciones marcadas para seguimiento.
        </p>
      </div>
      ...
      {!error && tracked.length === 0 && (
        <div className="rounded-lg border border-zinc-200 bg-white py-16 text-center">
          <p className="mb-4 text-sm text-zinc-500">
            No tienes licitaciones en seguimiento.
          </p>
          <Link
            href="/tenders"
      ...
      {!error && tracked.length > 0 && (
        <div className="space-y-4">
          <p className="text-xs text-zinc-400">
            {tracked.length} licitación{tracked.length === 1 ? "" : "es"} en
            seguimiento
          </p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {tracked.map((tender) => (
              <div
                key={tender.code}
                className="rounded-lg border border-zinc-200 bg-white transition-shadow hover:shadow-md"
              >
                {/* Header */}
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-xs">
                    ...
                    {tender.publishDate && (
                      <div>
                        ...
                        <p className="text-zinc-700">
                          {formatDate(tender.publishDate.toString())}
                        </p>
                      </div>
                    )}
                    {tender.closeDate && (
                      <div>
                        ...
                        <p className="text-zinc-700">
                          {formatDate(tender.closeDate.toString())}{" "}
                          <span className="text-zinc-400">
                            ({daysUntil(tender.closeDate.toString())})
                          </span>
                        </p>
                      </div>
                    )}
                    <div className="col-span-2">
                      ...
                      <p className="text-zinc-700">
                        {formatDate(tender.trackedAt.toString())}
                      </p>
                    </div>
                  </div>
                </Link>
